...
#     """
#     # Updated regex to handle SQL-style escaped single quotes
...
# # def mask_value(value, mask_char='*'):
# #     """
# #     Mask a value while preserving the first 3 characters and any special characters
# #     """
# #     if value is None or len(value.strip()) == 0:
# #         return value
...
# #     # Find the position of special characters like (alm)
# #     if ' (alm)' in inner_value:
# #         main_text = inner_value.replace(' (alm)', '')
# #         # Mask only the main text part
# #         masked = main_text[:3] + mask_char * (len(main_text) - 3)
# #         masked = masked + ' (alm)'
# #     else:
# #         # Normal masking for values without special characters
# #         masked = inner_value[:3] + mask_char * (len(inner_value) - 3)
    
# #     # Return the masked value with quotes
# #     return f"'{masked}'"
...
#     """Clean and properly quote a value"""
#     if value.lower() == 'null':
#         return value
...
#             elif i == 0:  # Another nomor_identitas or similar field
...
#                 pattern = r"(?<=[a-zA-Z0-9])'(?=[a-zA-Z0-9])" # matches single quote between word characters
#                 result = re.sub(pattern, '', masked)
#                 # Reapply quotes if original value was quoted
#                 if (group.strip().startswith("'") and group.strip().endswith("'")) or \
#                    (group.strip().startswith('"') and group.strip().endswith('"')):
#                     new_value = f"'{result}'"
#                 else:
#                     new_value = result
...
#                 if (group.strip().startswith("'") and group.strip().endswith("'")) or \
#                    (group.strip().startswith('"') and group.strip().endswith('"')):
#                     new_value = f"'{cleaned_value}'"
#                 else:
#                     new_value = cleaned_value
            
#             new_group_values.append(new_value)
           
#         before_insert = f"({','.join(new_group_values)})"
#         # pattern = r"(?<=[a-zA-Z0-9])'(?=[a-zA-Z0-9])" # matches single quote between word characters
#         # result = re.sub(pattern, '', before_insert)
#         new_values.append(before_insert)
